...
- **Headless TCC reality (no-GUI hosts):** the daemon re-fires the "data from other apps" prompt on EVERY fresh process start and hangs (port stays closed) until someone clicks Allow at the GUI — the grant never survives a respawn for any binary (uv or Developer-ID framework Python, both tested). So keep ONE long-lived granted process alive (KeepAlive) and do NOT voluntarily restart it headless. Detail + the refuted framework-Python experiment: memory `things-db-migration-tcc`, KB `KB-Agent-Tool-Access`.
- **`initialize` success does NOT prove reads work** (a daemon with a lost grant still answers `initialize` — the old watchdog's blind spot). To verify a real read from the shell: POST `initialize` capturing the `mcp-session-id` response header → POST `notifications/initialized` with it → POST `tools/call get_today` → grep for `unable to open database file` vs `"total":N`.
